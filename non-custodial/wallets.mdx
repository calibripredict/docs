...
An email-and-password sign-up creates your Calibri account. The wallet step that
follows is what sets the type: enrol a passkey and you get `3`, connect a wallet
you already use and you get `2`. Signing up with a passkey does both steps at once.

**A second signing key keeps you on type `3`.** Same Safe, same `maker`, same
`signer` — what changes is who can produce the signature. A passkey and a
browser-held session key sign only inside their browser; a key you hold signs
from a script.

You may also see `signature_type` `0` — `EOA`, meaning `maker` is your wallet
address with no Safe behind it. It appears in two situations: before you have
linked a wallet, and when Calibri could not derive your Safe on that request.
Read the endpoint again and sign for the type it returns.
