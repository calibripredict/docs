...
It is not a trading fee and it is not a percentage we set per deposit. It is capped by the
contract itself:
...
Those are ceilings, not estimates, and they assume Circle still charges nothing. The fee
charged is the gas cost at the time, and the Wallet page shows the current estimate before
you send.
...
| Pay **one capped fee** to Calibri's treasury, only when Calibri's relayer is the caller | Take more than 10 USDC, or more than 20% of the amount, per bridge |
...
Two addresses are configurable — the **relayer** allowed to charge the fee, and the
**treasury** it is paid to — and only by the fee-configuration admin. Everything that
...
| **Calibri's relayer** | The above, plus **one** fee per bridge of the whole balance, capped at 10 USDC and 20% | Take a fee on part of the balance; exceed the caps; redirect the USDC or the fee |
| **Fee-configuration admin** | Change the relayer and the treasury | Change the caps, the destination, the token or Circle's contract |
