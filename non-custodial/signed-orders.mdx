...
The signature type says what owns your Safe, and that decides how the exchange
checks your signature.

- **`2`** — your wallet owns the Safe. The exchange recovers your address from
  the signature and checks it derives your Safe.
- **`3`** — a passkey owns the Safe. An address cannot be recovered from a
  contract, so the exchange asks the Safe whether the signature is good.
- **`0`** — you have no Safe. Your wallet trades directly.

`GET /api/v2/atlas/account/wallet` returns your type as `signature_type`. Send
that value — another one will be rejected.

The names below are the exchange contract's enum. They mean the same as the
numbers:
