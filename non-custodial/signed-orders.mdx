...
description: "Build, sign, and place a non-custodial order — the EIP-712 Order domain and struct, 6-decimal unit rules, and the signed-order endpoint."
---
...
A signature type is not a choice. It says **what owns your Safe**, which
decides how the exchange can check a signature at all:

- **`2`** — your own wallet owns it. The exchange recovers your address from
  the signature and checks it derives your Safe.
- **`3`** — a passkey owns it. Nothing can be recovered from a contract, so the
  exchange asks the Safe itself whether the signature is good.
- **`0`** — there is no Safe. Your wallet trades directly.

`GET /api/v2/atlas/account/wallet` returns yours as `signature_type`, so you
never have to work it out. Send that value; sending a different one fails.

The names in the table are the exchange contract's own enum, and carry no
meaning beyond the number:
