...
description: "Limit vs market orders, choosing YES or NO, and how a fill becomes a position."
---
...
| **Price** | Price per contract, `0.01`–`0.99` (limit orders only). |

<Note>
  On the API these map to the order fields `market`, `side` (`"yes"`/`"no"`), `ord_type` (`"limit"`/`"market"`), `volume`, and `price`. See the [API Overview](/api-reference/introduction) for the exact request shape.
</Note>
...
A **market order** fills right away against the best available prices, walking the
book until your volume is filled.

```
Best resting liquidity for YES: 0.60, then 0.61, then 0.62 …
You place: BUY 100 YES, market
→ Fills against the cheapest available prices first.
→ Larger orders may fill across several price levels (slippage).
```

- **Pros:** immediate execution.
- **Cons:** you don't control the exact fill price; large orders can move the price.
...
- You can **exit before resolution** by trading the opposite side — the price
