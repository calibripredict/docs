...
| **CctpBridgeModule** | The module on every Safe. One entry point, `bridge(safe, amount, fee, maxCctpFee)`: it moves that network's USDC through **Circle's CCTP** to **the same Safe address on Polygon**. The destination is fixed in code, never a parameter. No storage, no owner, no upgrade path. On Polygon itself it is inert. |
| **BridgeFeeConfig** | Holds two addresses: the **relayer** allowed to charge the bridge fee, and the **treasury** the fee is paid to. Its admin can change those two and nothing else. |
...
| **Relayer** (set in BridgeFeeConfig) | Everything anyone can, plus one fee per whole-balance bridge, at most `min(10 USDC, 20%)`, paid to the treasury | Take a fee on part of the balance; exceed the caps; redirect the burn or the fee |
| **Fee-config admin** | Change the relayer and the treasury; hand the admin role over (two-step) | Change the fee caps, destination, token or Circle's messenger — all fixed in the module; renounce the role |
