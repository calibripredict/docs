...
| `This account's wallet is a passkey wallet, so orders must be signed by it…` | 422 | A type `0`/`2` order from an account whose wallet is passkey-owned. A linked wallet is a sign-in credential; to sign orders it must be added as a [signing key](/non-custodial/signing-keys) and sign as type `3` |
