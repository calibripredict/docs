...
| `market.order.missing_price` / `missing_volume` / `missing_market` | 422 | Required field absent |
...
| `market.order.insufficient_market_liquidity` | 422 | A market order cannot be filled to the requested size |
| `market.order.spend_amount_*` | 422 | Misuse of `spend_amount` / `max_spend` / `use_max_available` |
