...
<Tabs>
  ...
  <Tab title="2 or 0 — your own wallet">
    **Full access.** Your Safe is owned by an address whose private key you hold,
    ...
    `maker` is your Safe (type `2`) or the EOA itself (type `0`); `signer` is
    always the EOA. The same key signs `execTransaction`, so withdrawals and
    redemptions are scriptable too. See
